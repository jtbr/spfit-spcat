...
#define NDLINE 128
  ...
  try {
    luin = fopen(argv[1], "r");
    if (luin == NULL) exit(EXIT_FAILURE);
    /* read first line of keyfile */
    if (fgetstr(line, NDLINE, luin) <= 0) exit(EXIT_FAILURE);
    nn = NDQN + 2;
    for (k = 0; k < nn; ++k)
      dvec[k] = 0.;
    if (pcard(line, dvec, nn, NULL) < 2) exit(EXIT_FAILURE);
    nqn = (int)dvec[0]; jmax = (int) dvec[1];
    if (nqn > NDQN || nqn <= 0) exit(EXIT_FAILURE);
    for (k = 0; k < nqn; ++k) {
      key[k] = (int)dvec[k+2];
    ...
    while (fgetstr(line, NDLINE, luin) > 0) {
      ...
      now = (PRULE)calalloc(sizeof(RULE)); ++nrule;
    ...
    if (luin == NULL) exit(EXIT_FAILURE);
    if (luout == NULL) exit(EXIT_FAILURE);
    for(;;) {
