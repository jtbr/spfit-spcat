...
#define NDCON 20
  ...
  try
  {
    ...
    luin = file_helpers::open_input(cfil[einp]);
    luout = file_helpers::open_output(cfil[eiam], "w");
    lulist = file_helpers::open_output(cfil[emoi], "w");
    luxyz = file_helpers::open_output(cfil[exyz], "w");
...
int coords(FILE *lu, int noatm, double *coord, double *wt, int *nx, double *dist, double *theta, double *phi)
{
...
int gcalc(int natm, double *wt, double *coord, double *dcoord, double *gval, double *cval)
{
...
double determ(double *a, int ndd, int nr, int nc)
{ /* calculate inverse & determinant */
  ...
  if (n > nsav) {
    ...
    iperm = (int *) calalloc(n * sizeof(int));
    nsav = n;
    iperm[0] = 0;
  }
  det = 1.;
...
int iamsolv(double *cval, double *fc)
{
...
int pamsolv(int natm, double *x, double *gval, double *tval)
{
...
int micvt(double *cval, double *gval, double vmuz, double vmuy, int flg)
{
...
double xinterp(int n, double *tauval, double *xtrans)
{
  double ang, ang1, tt;
  int i, k, ii;
  tt = 0.;
