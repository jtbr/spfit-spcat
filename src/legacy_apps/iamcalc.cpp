...
#define NRHOK 5
  ...
  try
  {
    ...
    luin = file_helpers::open_input(cfil[eiam]);
    luout = file_helpers::open_output(cfil[eout], "w");
    kflg0 = 0;
    kflgm = -1;
    /* read input */
    ...
    /* save 2-top info */
    kmap = (short *)calalloc(nl * sizeof(short));
    kmap[0] = (short) nline2;
    ...
    sigma2 = (int *)calalloc(nl);
    nl = (nvib2 + 1) * sizeof(S2TOP);
    vmap = (S2TOP *)calalloc(nl);
    ivtop = (int *)calalloc((size_t) nvtop * sizeof(int));
    ...
    eigv = (SEIG *) calalloc(nl);
    /* allocate space for sigma */
    nl = (size_t) nvtot * sizeof(int);
    sigma = (int *) calalloc(nl);
    sigma[0] = 0;
    /* allocate space for morg + isym */
    isym = (int *) calalloc(nl);
    isym[0] = 0;
    for (k = 1; k < nvtot; ++k)
      isym[k] = 0;
    nl *= (size_t) neigp;
    pmorgwk = (int *) calalloc(nl);
    pmorgwk[0] = 0;
    nl = (size_t) nbasis;
    ...
    ioffv = (int *) calalloc(nl);
    ioffv[0] = 0;
    ixeig = (int *) calalloc(nl);
    ixeig[0] = 0;
    ixpt0 = (int *) calalloc(nl);
    ixpt0[0] = 0;
    nl = (size_t)ncalc * sizeof(double);
    rhokdif = (double *) calalloc(nl);
    rhokdif[0] = 0.;
    kk = 0;
    ...
    par = (SIPAR **) calalloc(nl);  /* allocate vector of parameter pointers */
    nl = (size_t) npar * sizeof(SIPAR);
    parwk = (SIPAR *) calalloc(nl); /* allocate parameter structures */
    par[0] = parwk;
    spar_alloc(parwk, par, npar);
    nl = (size_t) (ntyp + 1) * sizeof(SXPEC *);
    pxpecv = (SXPEC **) calalloc(nl);      /* allocate vector of expectation pointers */
    nl = (size_t) (ntyp + 1) * sizeof(SXPEC);
    pxpecpwk = (SXPEC *) calalloc(nl);     /* allocate expectation structures */
    pxpecv[0] = pxpecpwk;
    pxpecpwk->xpec = NULL;
    ...
    if (kmax >= 0) {
      ...
      eigvalk = (double *) calalloc((size_t) nvtot * nl);
      eigvalk[0] = 0.;
      eigvalk2 = (double *) calalloc((size_t) nvtot2 * nl);
      eigvalk2[0] = 0.;
    }
    pxpecp = pxpecv[0];
    pxpecp->nval = nsym;
    ...
    lubak = file_helpers::open_output(cfil[ebak], "w");
    headok = FALSE;
    if (lupar != NULL) {
    ...
    if (luint == NULL && ndip > 0)
      luint = file_helpers::open_output(cfil[eint], "w");
    nnpar = 0;
    nl = (size_t) (2 * nft) * sizeof(double);
    txpec = (double *) calalloc(nl);
    txpec[0] = 0.;
    /*   print out rho */
    if (kmax0 > 0 && !nofc) {
      val = 3. / nft; drhok = 1.;
    ...
    if (nvib2 > 0) {
      ...
      vmapsym = (int *)calalloc((size_t) ndmtop * sizeof(int));
      vmapsym[0] = 0;
      ...
      vmapsym = (int *) calalloc(sizeof(int)); vmapsym[0] = 0;
    }
    xpec2top = (double *) calalloc(sizeof(double));
    xpec2top[0] = 0.;
...
int iamdiag(FILE *lu, int nvib, SXPEC * *pxpecv, SEIG *eig, double *egy, int ioff)
{
  SXPEC *pxpec0;
  ...
  if (init == 0) { /* initialize */
    ...
    iassgn = (short *) calalloc(nl);
    iassgn[0] = 0;
    idx = (short *) calalloc(nl);
    idx[0] = 0;
    nl = (size_t) nbasis * sizeof(double);
    tegy = (double *) calalloc(nl);
    twk = (double *) calalloc(nl);
    nsq = nbasis * nbasis;
    nl = (size_t) nsq * sizeof(double);
    t = (double *) calalloc(nl);
    if (nxvib > 1) nl *= nxvib;
    tv = (double *) calalloc(nl);
...
int expec(int ityp, int indx, SXPEC * *pxpecv, int nvib, SEIG *eigm, SEIG *eigp, int noff, int *isym)
{
                  ...
                  int *pmorgwk, const int nvib)
{
  ...
  pmorg = pmorgwk;
  do {
    eig->eigvec = (double *) calalloc(nfill);
    eig->morg = pmorg;
    eig->irhok = 0;
    ++eig;
    pmorg += nvib;
