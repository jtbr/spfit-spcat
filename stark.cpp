...
#define NFILE 2
  ...
  try
  {
    ...
    lustr = file_helpers::open_input(fname[estr]);
    lustk = file_helpers::open_output(fname[estk], "w");
...
AB_STR *lposn(char *iqnu, int idx, int nn)
{
  AB_STR *now, *last;
  int i;
  ...
  if (head[2] == NULL) {
    now = (AB_STR *) calalloc(sizeof(AB_STR));
  } else {
    now = head[2];
    head[2] = now->next;
  }
...
int starks(double *a, double *b, double *c, int iqfx, int iqdel, int ifhalf)
{
  int idgn;
  double f, fsq, aa, den;
