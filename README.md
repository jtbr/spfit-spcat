...
The codebase has been refactored from monolithic C into a modular C++ architecture while preserving exact numerical compatibility with the 2008 reference binaries (as modified by Kelvin Lee) across a 12-molecule 55-file test suite. A C++ and Python API is now available for programmatic, in addition to the command line usage. Finally, a parallel [TOML-based file format](TOML.md) is available as a cleaner, more readable alternative to the legacy fixed-width ASCII files. The TOML files are human-readable representations of the same typed structs used by the API. See [here](TOML.md#cli-auto-detection) for more on usage.

**Command-line usage of all executables (spfit, spcat, etc.) is backwards compatible with the original C code. Existing input files and workflows work without modification.** The one exception is `dpcat` and `dpfit` have been folded into `spcat` and `spfit` respectively (use the argument `--dpi` to use the DPI engine matching the old behavior).
...
### Pre-built executables for command line usage
...
There is a very useful set of notes and usage information for the original library available [here](http://info.ifpan.edu.pl/~kisiel/asym/pickett/crib.htm). There is also a separate [python wrapper](https://github.com/Ltotheois/Pyckett) available for the original tools which should still work with the legacy-format mode of these executables.
...
subprocesses or writing intermediate files. Full documentation, including how to build the package,
is [here](python/README.md).
...
There are two essentially equivalent approaches.
...
To match the provided v2008 reference outputs exactly, the build must use the bundled `dblas.c` fallback BLAS (not OpenBLAS or other optimized BLAS) and disable FMA contraction:
...
**`Regression Tests`** — runs on every push to `main` and on pull requests. Builds with Makefile (`dblas.c` + `-ffp-contract=off`) as required for bit-identical comparison to the v2008 baseline, then runs:
...
4. Regenerates the pip index on the `gh-pages` branch (served via GitHub Pages for easy package install with pip)
...
**GitHub Pages setup**: To enable pip index, Pages needs to be deployed from the `gh-pages` branch, root folder.
...
Initially, leading LLMs and agents could often give reasonable feedback, but struggled to complete tasks correctly, frequently gave poor suggestions or changed things unnecessarily and frequently got into coding/debugging loops from which they couldn't escape. I often needed to step in and do things correctly or interrupt and redirect.
Managing context has been and remains an issue (though it is much easier with now-smaller file sizes). But LLMs and agents have developed impressively over the last year (Spring 2025 to Spring 2026), to the point where they are much more able to correctly complete large and complex tasks, to include debugging well-obscured bugs, nearly without correction. It has been a fruitful experiment. Hopefully the result will also prove useful.
