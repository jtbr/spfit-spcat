...
static SVIB vinfo1; /* Default/first vibrational state info */
/*@owned@*/ static SVIB *vinfo = &vinfo1; /* Pointer to array of vibrational state info */
...
static PSPAR spar_head[MAXVIB]; /* Array of linked list heads for parameters, one per vibrational state (iv2, the lower state in an interaction) */

static short sptzero[] = { 1, 0 }; /* Default spin table for a molecule with no spins: nset=1 (N only), 2*S=0 */
...
static SSP ssp_head = { NULL, sptzero, 1 , 0}; /* Head of linked list for unique spin patterns */

static struct {   /* Cached data for getqn function to speed up repeated calls for the same block */
...
static SDIP dipinfo0; /* Default/first dipole info structure */
/*@owned@*/ static SDIP *dipinfo = &dipinfo0; /* Pointer to array of dipole info structures */

static double zero = 0.;    /* Static zero value */
static double zwk = 0.;     /* Static zero for work array fallback */
static double spfac[MAXII]; /* Spin factors sqrt(I(I+1)(2I+1)) for quadrupole etc. */
static double spfac2[MAXII];/* Spin factors for quadrupole, involving 1/sqrt((I-1/2)(I+3/2)) etc. */
static int zmoldv = 0;      /* Default/zero for moldv fallback */
static int zblkptr = 0;     /* Default/zero for blkptr fallback */
static int zivs = 0;        /* Default/zero for ivs fallback */
static int zipder = 0;      /* Default/zero for ipder fallback */
static int revsym[] = { 0, 3, 2, 1}; /* Symmetry reversal map (e.g., Bx <-> Bz for oblate) */
static int isoddk[] = { 0, 1, 1, 0}; /* Indicates if K is effectively odd for symmetry types A, Bx, By, Bz for direction cosines */
static int ixphase[] = {0, 1, 2, 3}; /* Scratch for phase choices, modified by glob.stdphase */
static int ipwr2[]={0,1,2,4};        /* Powers of 2 (1,2,4) used for symmetry component checking (bit masks) */
static int is_esym[MAXITOT];         /* Array indicating E-symmetry for I_tot components: 0=A/B, 1=E_a, -1=E_b */
/* Angular momentum coupling arrays for a single operator */
static int lscom[MAXNS];  /* Tensor orders L for successive couplings in F = N+S+I1+I2... */
static int iscom[MAXNS];  /* 2*Angular momentum values for bra state (2N, 2J, 2F1...) */
static int jscom[MAXNS];  /* 2*Angular momentum values for ket state */
static int ismap[MAXNS];  /* Maps spin index to its position in iscom/jscom for tensor calculations */
/* Common blocks for rotational/rovibrational quanta storage */
static int ixcom[NDXCOM]; /* Stores quantum numbers for the 'bra' side of a matrix element */
static int jxcom[NDXCOM]; /* Stores quantum numbers for the 'ket' side of a matrix element */
/* Various global integer flags and counters */
static int itptr;         /* Index of I_tot in the spin coupling scheme */
static int itsym;         /* Index of the first spin summed into I_tot */
static int nspin;         /* Maximum number of spins encountered across all vibrational states */
static int nsqmax;        /* Maximum power of N(N+1) encountered for any parameter */
static int ndmx;          /* Maximum Hamiltonian matrix dimension encountered / work array size */
static int ndmax;         /* Maximum Delta N for any interaction */
static int nddip;         /* Number of dipole parameters currently allocated in dipinfo */
/* Static short variables, often default/zero values or fallbacks for unallocated pointers */
static short szero = 0;   /* Static zero short value */
static short zidx = 0;    /* Default/zero for idx fallback */
static short zjdx = 0;    /* Default/zero for jdx fallback */
static short ziqnsep = 0; /* Default/zero for iqnsep fallback */
static short zibkptr = 0; /* Default/zero for ibkptr fallback */
static short zikmin = 0;  /* Default/zero for ikmin fallback */

static struct { /* Global settings and state variables */
...
} glob; /* Global variables structure */
static char sbcd[NSBCD]; /* Buffer for BCD (Binary Coded Decimal) to string conversion */

/* Pointers to dynamically allocated arrays used throughout calculations */
/*@owned@*/ static int *moldv;   /* Array storing packed (Vib,Sym,SpinPattern) identifiers for each block */
/*@owned@*/ static int *blkptr;  /* Array of pointers to the start of each F-block group in moldv */
/*@owned@*/ static int *ipder;   /* Array mapping parameter index to its derivative column, or -ve for constrained */
/*@owned@*/ static int *ivs;     /* Array storing packed (Vib,Sym,SpinPattern) for each sub-block */
/*@owned@*/ static double *wk;   /* Main work array for Hamiltonian elements, direction cosines, etc. */
/*@owned@*/ static short *idx;   /* Row indices for sparse matrix elements (Hamiltonian or dipole) */
/*@owned@*/ static short *jdx;   /* Column indices for sparse matrix elements */
/*@owned@*/ static short *iqnsep;/* Array storing separation information for sorting/diagonalization, or K values for projection sort */
/*@owned@*/ static short *ibkptr;/* Array of pointers to the start of each sub-block (Wang block * spin) */
/*@owned@*/ static short *ikmin; /* Array of minimum K values for each sub-block */
